...
const FileResults: React.FC<FileResultsProps> = ({
  ...
  const handleFilterChange = (filter: SentimentFilter) => {
    /* ... (same) ... */ setFilterSentiment(filter);
    setCurrentPage(1);
  };
  const goToNextPage = () => {
    /* ... (same) ... */ setCurrentPage(currentPage + 1);
  };
  ...
  const getFilterButtonStyle = (filter: SentimentFilter) => {
    ...
    if (filter === filterSentiment) {
      ...
      // Inactive state: transparent/darker bg, lighter text on hover
  ...
  return (
    <section className="w-full max-w-6xl mt-12">
      ...
      {/* --- UPDATED Chart Section (Grid Layout) --- */}
            ...
            <ResponsiveContainer>
              <PieChart>
                <Pie
                  ...
                  cx="50%"
                  cy="50%"
                  outerRadius={120} // Made the radius a bit smaller
                  fill="#8884d8"
                  dataKey="value"
                  label={
                    ({ percent }: any) => `${(percent * 100).toFixed(0)}%` // Only show percentage
                  }
                  labelLine={false}
                >
          ...
          <div style={{ width: "100%", height: 350 }}>
            <ResponsiveContainer>
              ...
              >
                ...
                {/* Grid color (slate-600) */}
                <XAxis dataKey="name" tick={{ fill: "#cbd5e0" }} />{" "}
                {/* X-axis labels color (slate-300) */}
                <YAxis allowDecimals={false} tick={{ fill: "#cbd5e0" }} />{" "}
                {/* Y-axis labels color (slate-300) */}
                <Tooltip
                  cursor={{ fill: "rgba(100, 116, 139, 0.2)" }} // Tooltip background
                  // --- CSS UPDATED ---
                  contentStyle={{
                    backgroundColor: "#1e293b", // slate-800
                    border: "1px solid #334155", // slate-700
                  ...
                  labelStyle={{ color: "#e2e8f0" }} // Tooltip label color (slate-200)
                />
      ...
      <div className="w-full bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
        ...
        {/* Pagination Controls (CSS UPDATED) */}
        {totalPages > 1 && (
